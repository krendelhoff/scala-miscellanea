...
object PersistentModel extends Model:
  ...
  def update(id: Id)(f: Task => Task): Option[Task] = {
    ...
    val updatedTasks = tasks.updatedWith(id)(opt => opt.map(f))
  ...
  def delete(id: Id): Boolean =
    ...
    tasks.get(id) match {
      case None => false
      case _    => saveTasks(Tasks(tasks - id))
                   true
    }
  ...
  def tasks(tag: Tag): Tasks =
    Tasks(this.tasks.toList.filter((_,v) => v.tags.contains(tag)))


  def complete(id: Id): Option[Task] =
    val tasks = loadTasks().toMap
    val updatedTasks = tasks
      .updatedWith(id)(opt => opt.map(_.copy(state = completedNow)))
    saveTasks(Tasks(updatedTasks))
    updatedTasks.get(id)

  def tags: Tags =
    Tags(this.tasks.toList.map((_,v) => v).flatMap(_.tags).distinct)
