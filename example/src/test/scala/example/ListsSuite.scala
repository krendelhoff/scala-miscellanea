...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of a zero numbers (10pts)") {
    assertEquals(sum(List()), 0)
  }
  test("sum of a zeros (10pts)") {
    assertEquals(sum(List(0,0,0,0)), 0)
  }
  test("sum of a negative and positive (10pts)") {
    assertEquals(sum(List(-1,1,-1,1)), 0)
  }
  test("sum of best numbers (10pts)") {
    assertEquals(sum(List(1337,42,34,69)), 1482)
  }
  ...
  test("max of empty list (10pts)") {
    try
      max(List())
      fail("Exceptions wasn't thrown")
    catch
      case e: NoSuchElementException => ()
  }
  test("max of best numbers list (10pts)") {
    assertEquals(max(List(1337,42,34,69)), 1337)
  }
