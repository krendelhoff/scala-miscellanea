 ...
 */
case class Waiting( countDown: Int, startPosition: Point
                  , numberOfParticles: Int, particlesColor: Color
                  ) extends Firework:
 ...
 * @param particlesColor    color of this firework
 */
case class Launched( countDown: Int, position: Point, direction: Angle
                   , numberOfParticles: Int, particlesColor: Color
                   ) extends Firework:
  /**
   * @return The next state of this firework
  ...
  def next: Firework =
    if countDown <= 0
    then Exploding.init(numberOfParticles, direction, position, particlesColor)
    else Launched( countDown - 1
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
   ...
   *       of this firework.
   */
  def next: Firework = if countDown <= 0
                       then Done
                       else Exploding(countDown - 1, particles.next)
