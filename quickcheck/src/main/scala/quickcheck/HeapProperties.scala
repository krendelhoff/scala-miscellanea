...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val max = if x < y then y else x 
      val min = if x < y then x else y
      val meldedHeap: List[Node] = meld(insert(max, insert(max, empty)), insert(min, insert(min, empty)))
  ...
  val meldingHeaps: (String, Prop) =
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      ...
      fix[List[Node] => List[Node] => List[Node] => Boolean]
        (rec => m => h1 => h2 => (m, h1, h2) match {
          case (Nil, _, _)    => true
          case (_, Nil, _) => true
          case (_, _, Nil) => true
          case (melded, h1, h2) =>
            ...
            if minH1 < minH2 then (findMin(melded) == minH1) && rec(deleteMin(h1))(h2)
            else (findMin(melded) == minH2) && rec(h1)(deleteMin(h2))
        })(melded)(heap1)(heap2)
    }
